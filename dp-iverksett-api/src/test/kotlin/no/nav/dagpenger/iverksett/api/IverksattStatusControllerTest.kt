...
class IverksattStatusControllerTest {
    ...
    fun `skal svare med 200 når person har iverksatt vedtak`() {
        val vedtaksdetaljerDagpenger = vedtaksdetaljerDagpenger()
        every { vedtakStatusServiceMock.getVedtakStatus(any()) } returns vedtaksdetaljerDagpenger

        val response = iverksattStatusController.hentStatusForPerson("12345678910")

        assertEquals(HttpStatus.OK, response.statusCode)
        assertEquals(vedtaksdetaljerDagpenger, response.body)
    }
}
